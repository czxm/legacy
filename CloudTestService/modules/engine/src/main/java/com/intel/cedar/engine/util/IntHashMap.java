...
public class IntHashMap implements Serializable {
     ...
     * the arrays in which keys and values are stored. For efficiency, only a
     * fraction of the elements in those arrays are used. That fraction is the
     * specified load factor. The initial length of the arrays equals the
     * smallest power of two not less than the ratio capacity/factor. The
     * capacity of the map is increased, as necessary. The maximum number of
     * keys that can be mapped is 2^30.
     * 
     * @param capacity
     *            the initial capacity.
     * @param factor
    ...
    public void put(int key, Object value) {
        if (value == null) {
            throw new NullPointerException(
                    "IntHashMap does not allow null values");
        }
        int i = indexOf(key);
        // if (_filled[i]) {
        if (_value[i] != null) {
    ...
    private void setCapacity(int capacity) {
        ...
        double factor = (_factor < 0.01) ? 0.01 : (_factor > 0.99) ? 0.99
                : _factor;
        int nbit, nmax;
