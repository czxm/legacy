...
public class Whitespace {
    ...
    /**
     * The values PRESERVE, REPLACE, and COLLAPSE represent the three options
     * for whitespace normalization. They are deliberately chosen in ascending
     ...
     * text node should be stripped when building a source tree. UNSPECIFIED
     * indicates that no particular request has been made. XSLT indicates that
     * whitespace should be stripped as defined by the xsl:strip-space and
     ...
     * @return true if the character is one of tab, newline, carriage return, or
     *         space
     */
     ...
     * 
     * @param action
     *            the action to be applied: one of PRESERVE, REPLACE, or
     *            COLLAPSE
     * @param value
     ...
     * @return true if the string contains a character that is XML whitespace,
     *         that is tab, newline, carriage return, or space
     */
    ...
    public static boolean isWhite(CharSequence content) {
        ...
        for (int i = 0; i < len;) {
            ...
            // characters, are <= 0x20
            // But XML 1.1 allows non-white characters that are also < 0x20, so
            // we need a specific test for these
    ...
    /**
     * Normalize whitespace as defined in XML Schema. Note that this is not the
     * same as the XPath normalize-space() function, which is supported by the
     * {@link #collapseWhitespace} method
     * 
     * @param in
     *            the string to be normalized
     * @return a copy of the string in which any whitespace character is
     ...
     * @return the string with any leading or trailing whitespace removed, and
     *         any internal sequence of whitespace characters replaced with a
     ...
     * This differs from the Java trim() method in that the only characters
     * treated as whitespace are space, \n, \r, and \t. The String#trim() method
     * removes all C0 control characters (which is not the same thing under XML
     * 1.1).
     * 
     * @param s
     *            the string to be trimmed. If null is supplied, null is
     *            returned.
