...
import com.intel.soak.plugin.hcatalog.conf.TableSchemaXMLParser;
...
public class HCatMixSetup extends Configured implements Tool {
    ...
    public static void main(String[] args) throws Exception {
        ToolRunner.run(new Configuration(), new HCatMixSetup("user1", null, false), args);
    }
    ...
    public HCatMixSetup(String user, String metastore, boolean splitHMS) throws MetaException {
        HiveConf hiveConf = new HiveConf(HCatMixSetup.class);
        if(metastore != null)
            hiveConf.set("hive.metastore.uris", metastore);
        if(splitHMS){
            String hms = hiveConf.get("hive.metastore.uris");
            String[] uris = hms.split(",");
            if(uris.length > 1){
                String userIndex = user.replaceAll("[a-zA-Z]","");
                Integer ui = Integer.parseInt(userIndex);
                hiveConf.set("hive.metastore.uris", uris[ui % uris.length]);
            }
        }
        LOG.info(hiveConf.get("hive.metastore.uris"));
    ...
    public void setupFromConf(HCatMixSetupConf conf)
        ...
        for (HiveTableSchema hiveTableSchema : multiInstanceList) {
            if(conf.isCreateTable()) {
                try {
                    hiveTableSchema.setName(user + "_" + hiveTableSchema.getName());
    ...
    public void createTable(HiveTableSchema hiveTableSchema) throws IOException, TException, NoSuchObjectException, MetaException, AlreadyExistsException, InvalidObjectException {
        if(hiveClient.tableExists(hiveTableSchema.getDatabaseName(), hiveTableSchema.getName())){
            hiveClient.dropTable(hiveTableSchema.getDatabaseName(), hiveTableSchema.getName());
            LOG.info("Drop table: {0}", hiveTableSchema.getName());
        }
        Table table = new Table();
        table.setDbName(hiveTableSchema.getDatabaseName());
        LOG.info(hiveTableSchema.getDatabaseName());
        table.setTableName(hiveTableSchema.getName());
        try {
            table.setOwner(UserGroupInformation.getCurrentUser().getUserName());
            LOG.info(UserGroupInformation.getCurrentUser().getUserName());
        } catch (IOException e) {
    ...
    public Partition addPartition(HiveTableSchema hiveTableSchema) throws TException, IOException {
        String tableName = hiveTableSchema.getName();
        if(!tableName.contains("no")){
            Table table = new Table();
            table.setDbName(hiveTableSchema.getDatabaseName());
            LOG.info(hiveTableSchema.getDatabaseName());
            table.setTableName(hiveTableSchema.getName());
            ...
            table.setOwnerIsSet(true);
            String dbName = hiveTableSchema.getDatabaseName();
            Table t = new Table();
            ...
            LOG.info(MessageFormat.format("About to add partition: {0}.{1}", dbName, tableName));
            try {
                hiveClient.add_partition(part);
            }catch(AlreadyExistsException e){
                LOG.info(MessageFormat.format("Partition already exists: {0}.{1}", dbName, tableName));
            }
            LOG.info(MessageFormat.format("Successfully added partition: {0}.{1}", dbName, tableName));
            return part;
        }
        return null;
    }

    public void dropPartition(String dbName, String tableName, Partition part) throws TException {
        if(!tableName.contains("no") || part != null){
            LOG.info(MessageFormat.format("About to drop partition: {0}.{1}", dbName, tableName));
            hiveClient.dropPartition(dbName, tableName, part.getValues(), false);
            LOG.info(MessageFormat.format("Successfully dropped partition: {0}.{1}", dbName, tableName));
        }
    }
