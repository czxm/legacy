 ...
 */
abstract public class AbstractModelAdapterFactory implements
     ...
     */
    protected AbstractModelAdapterFactory() {
        super();

    }

    public AbstractModelAdapterFactory(Object adapterKey) {
    ...
    /**
     * ISSUE: should be final. See those that implement it for complicating
     * details and "unknowns".
     */
    public IModelAdapter adapt(IModelNotifier target) {
    ...
    /**
     * Subclasses should normally implement their own 'copy' method. By default,
     * we'll return the same instance, for convenience of those using singleton
     * factories (which have no state, and so need to do anything on 'release').
     * 
     */
    public IModelAdapterFactory copy() {
        return this;
    }

    /**
     * This method needs to return true of this factory is for adapters of type
     * 'type'. It is required that it return true if 'equals' and this default
     * behavior is provided by this super class. Clients may extend this
     * behavior if more complex logic is required.
     */
    ...
    /**
     * Subclasses may need to "cleanup" their adapter factory, release adapters,
     * resources, etc. Subclasses may extend this method if such clean up is
     * required. Note: while current behavior is to do nothing, subclasses
     * should not assume this would always be true, so should always call
     * super.release at the end of their method.
     */
    public void release() {
    ...
    final protected void setAdapterKey(Object key) {
        if (fAdapterKey != null && !(fAdapterKey.equals(key)))
            throw new IllegalStateException(
                    "IModelAdapter Key cannot be changed."); //$NON-NLS-1$
        fAdapterKey = key;
    }
     ...
     * @return
     */
    protected final IModelAdapter adaptNew(IModelNotifier target) {
        IModelAdapter adapter = createAdapter(target);
        if (adapter != null) {
     ...
     * or may not be relevent when creating the adapter. Note: the adapter does
     * not have to literally be a new intance and is actually recommended to
     * typically be a singleton for performance reasons.
     * 
     * @param target
     * @return
     */
    abstract protected IModelAdapter createAdapter(IModelNotifier target);
