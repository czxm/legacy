...
public interface IModelAdapterFactory {
     ...
     * associate it with an INodeNotifier. This adapt method Method that returns
     * the adapter associated with the given object. The implementation of this
     * method should call addAdapter on the adapted object with the correct
     * instance of the adapter, if appropriate.
     * 
     ...
     */
    IModelAdapter adapt(IModelNotifier object);
     ...
     * factory is apporiate to use for an adapter request that specifies 'type'.
     * 
     * @param type
    ...
    /**
     * release is called by infrastructure when the factory registry is released
     * (which is done when a structured model is released). This intened for the
     * factory to be allowed to clean up any state information it may have.
     * 
     * Note: while not recommended, due to performance reasons, if individual
     * adapters need some cleanup (or need to be released) it is (typically) the
     * responsibility of the adapter factory to track them, and initiate what
     * ever clean up is needed. In other works, cleanup at the adatper level is
     * not provided by infrastructure.
     */
    public void release();
