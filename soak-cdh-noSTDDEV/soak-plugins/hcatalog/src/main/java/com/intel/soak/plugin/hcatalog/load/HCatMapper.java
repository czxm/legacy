...
import static com.intel.soak.plugin.hcatalog.load.HadoopLoadGenerator.Conf;

/**
 * The mapper task that runs multples threads in parallel and executes the {@link com.intel.soak.plugin.hcatalog.load.tasks.Task} provided in
...
public class HCatMapper extends MapReduceBase implements
    ...
    public void map(LongWritable longWritable, Text text,
        ...
        // Initialize tasks
        List<com.intel.soak.plugin.hcatalog.load.tasks.Task> tasks;
        try {
    ...
    /**
     * Creates the {@link com.intel.soak.plugin.hcatalog.load.tasks.Task} instances using reflection and calls configure on it, The task names
     * are comma separated list of {@link com.intel.soak.plugin.hcatalog.load.tasks.Task} classes.
     * @param jobConf
     * @return
     * @throws Exception
     */
    private List<com.intel.soak.plugin.hcatalog.load.tasks.Task> initializeTasks(JobConf jobConf) throws Exception {
        ...
        List<com.intel.soak.plugin.hcatalog.load.tasks.Task> tasks = new ArrayList<com.intel.soak.plugin.hcatalog.load.tasks.Task>();

        String[] classes = classNames.split(",");
        for (String aClass : classes) {
            Class clazz = Class.forName(aClass);
            try {
                com.intel.soak.plugin.hcatalog.load.tasks.Task task = (com.intel.soak.plugin.hcatalog.load.tasks.Task) clazz.newInstance();
