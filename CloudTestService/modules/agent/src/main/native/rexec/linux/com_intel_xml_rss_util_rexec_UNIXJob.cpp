...
#include "com_intel_xml_rss_util_rexec_UNIXJob.h"
/*
...
{
    field_exitcode = env->GetFieldID( clazz, "exitcode", "I");
...
Java_com_intel_xml_rss_util_rexec_UNIXJob_waitForProcessExit(JNIEnv* env,
                                              jobject junk,
                                              jint pid)
{
...
JNIEXPORT jint JNICALL
Java_com_intel_xml_rss_util_rexec_UNIXJob_forkAndExec(JNIEnv *env,
                                       jobject process,
                                       jbyteArray prog,
                                       ...
                                       jboolean redirectErrorStream)
{
    ...
    assert(std_fds != NULL);
    fds = env->GetIntArrayElements( std_fds, NULL);
 ...
 Finally:
    ...
    if (fds != NULL)
        env->ReleaseIntArrayElements( std_fds, fds, 0);
...
(JNIEnv *env, jclass junk, jint pid)
{
    kill(-pid, SIGTERM);
}
