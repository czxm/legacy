...
/**
 * ItemType is an interface that allows testing of whether an Item conforms to
 * an expected type. ItemType represents the types in the type hierarchy in the
 * XPath model, as distinct from the schema model: an item type is either item()
 * (matches everything), a node type (matches nodes), an atomic type (matches
 * atomic values), or empty() (matches nothing). Atomic types, represented by
 * the class AtomicType, are also instances of SimpleType in the schema type
 * heirarchy. Node Types, represented by the class NodeTest, are also Patterns
 * as used in XSLT.
 * 
