...
import com.intel.cedar.engine.util.CharSlice;
...
/**
 * ReceiverContentHandler is a glue class that provides a standard SAX
 * ContentHandler interface to a Saxon Receiver. To achieve this it needs to map
 * names supplied as strings to numeric name codes, for which purpose it needs
 * access to a name pool. The class also performs the function of assembling
 * adjacent text nodes.
 * <p>
 * The class was previously named ContentEmitter.
 * </p>
 * <p>
 * If the input stream contains the processing instructions assigned by JAXP to
 * switch disable-output-escaping on or off, these will be reflected in
 * properties set in the corresponding characters events. In this case adjacent
 * text nodes will not be combined.
 * 
 * @author Michael H. Kay
 */

public class ReceiverContentHandler implements ContentHandler, LexicalHandler,
        DTDHandler {
    ...
    // buffer for accumulating character data, until the next markup event is
    // received
    ...
    // indicate that escaping is allowed to be disabled using the JAXP-defined
    ...
    /**
     * A local cache is used to avoid allocating namecodes for the same name
     * more than once. This reduces contention on the NamePool. This is a
     * two-level hashmap: the first level has the namespace URI as its key, and
     * returns a HashMap which maps lexical QNames to integer namecodes.
     */
    ...
    /**
     * Create a ReceiverContentHandler and initialise variables
     */

    public ReceiverContentHandler() {
    }

    /**
     * Set the ReceiverContentHandler to its initial state, except for the local
     * name cache, which is retained
     */

    public void reset() {
        receiverConfiguration = null;
        pool = null;
        receiver = null;
    ...
    /**
     * Set the receiver to which events are passed. ReceiverContentHandler is
     * essentially a translator that takes SAX events as input and produces
     * Saxon Receiver events as output; these Receiver events are passed to the
     * supplied Receiver
     * 
     * @param receiver
     ...
     * 
     * @param receiverConfiguration
     *            the pipeline configuration. This holds a reference to the
     *            Saxon configuration, as well as information that can vary from
     *            one pipeline to another, for example the LocationProvider
     *            which resolves the location of events in a source document
     */
    ...
    public void setReceiverConfiguration(
        ...
        this.receiverConfiguration.setLocationProvider(localLocator);
     ...
     *         {@link #setPipelineConfiguration(ReceiverConfiguration)}
     */
    ...
    public Configuration getConfiguration() {
        return receiverConfiguration.getConfiguration();
    }

    /**
     * Set whether "ignorable whitespace" should be ignored. This method is
     * effective only if called after setPipelineConfiguration, since the
     * default value is taken from the configuration.
     * 
     * @param ignore
     *            true if ignorable whitespace (whitespace in element content
     *            that is notified via the
     *            {@link #ignorableWhitespace(char[], int, int)} method) should
     *            be ignored, false if it should be treated as ordinary text.
     */
    ...
    /**
     * Determine whether "ignorable whitespace" is ignored. This returns the
     * value that was set using {@link #setIgnoreIgnorableWhitespace} if that
     * has been called; otherwise the value from the configuration.
     * 
    ...
    public void startDocument() throws SAXException {
        // System.err.println("ReceiverContentHandler#startDocument");
        try {
            charsUsed = 0;
            namespacesUsed = 0;
            receiverConfiguration.setLocationProvider(localLocator);
            receiver.setReceiverConfiguration(receiverConfiguration);
            receiver.open();
    ...
    /**
     * Supply a locator that can be called to give information about location in
    ...
    public void startElement(String uri, String localname, String rawname,
        ...
        try {
            ...
            receiver.startElement(nameCode, StandardNames.XS_UNTYPED, 0,
            ...
            for (int a = 0; a < atts.getLength(); a++) {
                ...
                // TODO: JDK15: eliminate use of reflection for
                    ...
                    if (suppressDTDAttributeDefaults) {
                        if (attributes2class.isAssignableFrom(atts.getClass())) {
                            try {
                                ...
                                Boolean specified = (Boolean) isSpecifiedMethod
                                        .invoke(atts, new Object[] { qname });
                            ...
                            // XML parser doesn't report whether attributes were
                            // defaulted, so we give up
                            suppressDTDAttributeDefaults = false;
                        }
                    }
                }
                int attCode = getNameCode(atts.getURI(a), atts.getLocalName(a),
                        atts.getQName(a));
                ...
                if (retainDTDAttributeTypes) {
                    ...
                    if (type.equals("ID")) {
                        typeCode = StandardNames.XS_ID | Node.IS_DTD_TYPE;
                    } else if (type.equals("IDREF")) {
                        typeCode = StandardNames.XS_IDREF | Node.IS_DTD_TYPE;
                    } else if (type.equals("IDREFS")) {
                        typeCode = StandardNames.XS_IDREFS | Node.IS_DTD_TYPE;
                    }
                }

                receiver.attribute(attCode, typeCode, atts.getValue(a), 0,
                        properties);
            }
    ...
    /**
     * Get the NamePool name code associated with a name appearing in the
     * document
     * 
     * @param uri
     *            the namespace URI
     ...
     * @throws SAXException
     *             if the information supplied by the SAX parser is insufficient
     */

    private int getNameCode(String uri, String localname, String rawname)
            throws SAXException {
        // System.err.println("URI=" + uri + " local=" + " raw=" + rawname);
        // The XML parser isn't required to report the rawname (qname), though
        // all known parsers do.
        // If none is provided, we give up
        if (rawname.length() == 0) {
            throw new SAXException(
                    "Saxon requires an XML parser that reports the QName of each element");
        }
        // It's also possible (especially when using a TransformerHandler) that
        // the parser
        ...
        // Following code maintains a local cache to remember all the namecodes
        // that have been
        // allocated, which reduces contention on the NamePool. It also avoid
        // parsing the lexical QName
        // when the same name is used repeatedly. We also get a tiny improvement
        // by avoiding the first hash
        // table lookup for names in the null namespace.

        HashMap map2 = (uri.length() == 0 ? noNamespaceMap : (HashMap) cache
                .get(uri));
        if (map2 == null) {
            map2 = new HashMap(50);
            cache.put(uri, map2);
        ...
        // we use the rawname (qname) rather than the local name because we want
        // a namecode rather than
    ...
    public void endElement(String uri, String localname, String rawname)
            throws SAXException {
        // System.err.println("ReceiverContentHandler#End element " + rawname);
        try {
            flush();
            receiver.endElement();
        } catch (XMLException err) {
    ...
    /**
     * Report character data. Note that contiguous character data may be
     * reported as a sequence of calls on this method, with arbitrary boundaries
     */

    public void characters(char ch[], int start, int length)
            throws SAXException {
        // System.err.println("characters (" + length + ")");
        // need to concatenate chunks of text before we can decide whether a
        // node is all-white
    ...
    /**
     * Report character data classified as "Ignorable whitespace", that is,
     * whitespace text nodes appearing as children of elements with an
    ...
    public void processingInstruction(String name, String remainder)
        ...
        try {
            ...
            if (!inDTD) {
                if (name == null) {
                    // trick used by the old James Clark xp parser to notify a
                    // comment
                    comment(remainder.toCharArray(), 0, remainder.length());
                } else {
                    // some parsers allow through PI names containing colons
                    if (!Name11Checker.getInstance().isValidNCName(name)) {
                        throw new SAXException(
                    ...
                    if (allowDisableOutputEscaping) {
                        if (name.equals(Result.PI_DISABLE_OUTPUT_ESCAPING)) {
                            ...
                            return;
                                ...
                                .equals(Result.PI_ENABLE_OUTPUT_ESCAPING)) {
                            flush();
                            escapingDisabled = false;
                            return;
                        }
                    }
                    receiver.processingInstruction(name, Whitespace
                            .removeLeadingWhitespace(remainder), 0, 0);
                }
            }
        } catch (XMLException err) {
            throw new SAXException(err);
    ...
    /**
     * Notify the existence of a comment. Note that in SAX this is part of
     * LexicalHandler interface rather than the ContentHandler interface.
     */
    ...
    private void flush() throws XMLException {
        if (charsUsed > 0) {
            slice.setLength(charsUsed);
            receiver.characters(slice, 0,
                    escapingDisabled ? ReceiverOptions.DISABLE_ESCAPING
                            : ReceiverOptions.WHOLE_TEXT_NODE);
            charsUsed = 0;
            escapingDisabled = false;
        }
    }
    ...
    /**
     * Register the start of the DTD. Saxon ignores the DTD; however, it needs
     * to know when the DTD starts and ends so that it can ignore comments in
     * the DTD, which are reported like any other comment, but which are skipped
     * because they are not part of the XPath data model
     */
    ...
    public void unparsedEntityDecl(String name, String publicId,
        ...
        // Some SAX parsers report the systemId as written. We need to turn it
        // into
        // an absolute URL.
    ...
    private class LocalLocator implements LocationProvider {
        ...
        /**
         * Get the line number within the document or module containing a
         * particular location
         * 
         * @param locationId
         *            identifier of the location in question (as passed down the
         *            Receiver pipeline)
        ...
        /**
         * Get the URI of the document or module containing a particular
         * location
         * 
         * @param locationId
         *            identifier of the location in question (as passed down the
...
} // end of class ReceiverContentHandler
