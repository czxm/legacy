...
/**
 * EmptyIterator: an iterator over an empty sequence. Since such an iterator has
 * no state, only one instance is required; therefore a singleton instance is
 * available via the static getInstance() method.
 */

public class EmptyIterator implements AxisIterator, LookaheadIterator {
     ...
     * 
     * @return an EmptyIterator (in practice, this always returns the same one)
     */
    ...
    /**
     * Get the current item, that is, the item returned by the most recent call
     * of next().
     * 
     ...
     * 
     * @return the position of the current item. For the EmptyIterator this is
     *         always zero (whether or not the next() method has been called).
     */
    public int position() {
        return 0;
    }
     ...
     * 
     * @return another iterator over an empty sequence (in practice, it returns
     *         the same iterator each time)
     */
