...
import com.intel.cedar.engine.xml.iterator.DocumentNodeTest;
 ...
 * Because such questions are potentially expensive, it caches the answers.
 * There is one instance of this class for a Configuration.
 */
...
public class TypeHierarchy implements Serializable {
    ...
    // private String[] relnames = {"SAME", "SUBSUMES", "SUBSUMED_BY",
    // "OVERLAPS", "DISJOINT"};
     ...
     * @return {@link #SAME_TYPE} if the types are the same; {@link #SUBSUMES}
     *         if the first type subsumes the second (that is, all instances of
     *         the second type are also instances of the first);
     *         {@link #SUBSUMED_BY} if the second type subsumes the first;
     *         {@link #OVERLAPS} if the two types overlap (have a non-empty
     *         intersection, but neither subsumes the other); {@link #DISJOINT}
    ...
    private int computeRelationship(ItemType t1, ItemType t2) {
        ...
        if (t1 instanceof AnyItemType) {
            ...
            if (t2 instanceof NodeTest) {
                ...
                if (((AtomicType) t1).getFingerprint() == ((AtomicType) t2)
                        .getFingerprint()) {
                    return SAME_TYPE;
                }
                ItemType t = t2;
                while (t.isAtomicType()) {
                    if (((AtomicType) t1).getFingerprint() == ((AtomicType) t)
                            .getFingerprint()) {
                        return SUBSUMES;
                    }
                ...
                t = t1;
                while (t.isAtomicType()) {
                    if (((AtomicType) t).getFingerprint() == ((AtomicType) t2)
                            .getFingerprint()) {
                        return SUBSUMED_BY;
                    }
                    t = t.getSuperType(this);
            ...
            if (t2.isAtomicType()) {
                ...
                if (t1 instanceof AnyNodeTest) {
                    ...
                    // first find the relationship between the node kinds
                    // allowed
                    ...
                    // Note that although
                    // NamespaceTest and LocalNameTest are NodeTests, they do
                    // not occur in SequenceTypes,
                    // so we don't need to consider them.
                    int nodeNameRelationship;
                    IntHashSet n1 = ((NodeTest) t1).getRequiredNodeNames(); // null
                                                                            // means
                                                                            // all
                                                                            // names
                                                                            // allowed
                    IntHashSet n2 = ((NodeTest) t2).getRequiredNodeNames(); // null
                                                                            // means
                                                                            // all
                                                                            // names
                                                                            // allowed
                    ...
                    // now find the relationship between the content types
                    // allowed
                    ...
                    if (t1 instanceof DocumentNodeTest) {
                        if (t2 instanceof DocumentNodeTest) {
                            contentRelationship = relationship(
                                    ((DocumentNodeTest) t1).getElementTest(),
    ...
    /**
     * Test whether a type annotation code represents the type xs:ID or one of
     * its subtypes
     * 
     * @param typeCode
     *            the type annotation to be tested
    ...
    public boolean isIdCode(int typeCode) {
        ...
        if (typeCode == StandardNames.XS_ID) {
            ...
            if (type == null) {
                return false; // this shouldn't happen, but there's no need to
                              // crash right here
            }
            ...
            if (type instanceof ComplexType
                ...
                SimpleType contentType = ((ComplexType) type)
                        .getSimpleContentType();
                if (contentType.isAtomicType()) {
                    return isSubType((AtomicType) contentType,
    ...
    /**
     * Test whether a type annotation code represents the type xs:IDREF,
     * xs:IDREFS or one of their subtypes
     * 
     * @param typeCode
     *            the type annotation to be tested
     * @return true if the type annotation represents an xs:IDREF or xs:IDREFS
     *         or a subtype thereof
     */

    public boolean isIdrefsCode(int typeCode) {
        typeCode &= NamePool.FP_MASK;
        if (typeCode == StandardNames.XS_IDREF
     ...
     *         {@link net.sf.saxon.type.TypeHierarchy#SAME_TYPE},
     *         {@link net.sf.saxon.type.TypeHierarchy#SUBSUMES},
     *         {@link net.sf.saxon.type.TypeHierarchy#SUBSUMED_BY},
     *         {@link net.sf.saxon.type.TypeHierarchy#DISJOINT}
     */
