...
public class SimpleHttpDriver extends GenericDriver {
	static{
    	try {
    		DefaultBootstrap.bootstrap ();	// openSAML
    		GenSecurityID.Initialize();		// random security id generator
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
	}
	...
	private List<ParamType> globalParams = new ArrayList<ParamType>();
	...
	public List<Transaction> createTransactions(List<TransactionType> transactions) {
		...
		for(TransactionType tt : transactions){
			try{
				...
				Transaction tran = (Transaction)con.newInstance(client, params);
				trans.add(tran);
			}
			catch(Throwable t){
				t.printStackTrace();
