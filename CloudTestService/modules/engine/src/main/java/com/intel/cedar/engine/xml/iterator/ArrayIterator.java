...
/**
 * ArrayIterator is used to enumerate items held in an array. The items are
 * always held in the correct sorted order for the sequence.
 * 
 * @author Michael H. Kay
 */

public class ArrayIterator implements SequenceIterator, ReversibleIterator,
        LookaheadIterator {
    ...
    protected int end; // position of first item that is NOT returned,
                       // zero-based
    ...
    public ArrayIterator(Node[] nodes) {
        items = nodes;
        start = 0;
        end = nodes.length;
        index = 0;
    ...
    /**
     * Create an iterator over a range of an array. Note that the start position
     * is zero-based
     * 
     * @param items
     *            the array (of nodes or simple values) to be processed by the
     *            iterator
     * @param start
     *            the position of the first item to be processed (numbering from
     *            zero). Must be between zero and nodes.length-1; if not,
     *            undefined exceptions are likely to occur.
     * @param end
     *            position of first item that is NOT returned, zero-based. Must
     *            be beween 1 and nodes.length; if not, undefined exceptions are
     *            likely to occur.
     */

    public ArrayIterator(Node[] items, int start, int end) {
        this.items = items;
        this.end = end;
        this.start = start;
        index = start;
    ...
    /**
     * Create a new ArrayIterator over the same items, with a different start
     * point and end point
     * 
     * @param min
     *            the start position (1-based) of the new ArrayIterator relative
     *            to the original
     * @param max
     *            the end position (1-based) of the last item to be delivered by
     *            the new ArrayIterator, relative to the original. For example,
     *            min=2, max=3 delivers the two items ($base[2], $base[3]). Set
     *            this to Integer.MAX_VALUE if there is no end limit.
    ...
    public SequenceIterator makeSliceIterator(int min, int max) {
        ...
        int newEnd = (max == Integer.MAX_VALUE ? currentEnd : newStart
                + (max - min + 1));
     ...
     * 
     * @return the number of items; equivalently, the position of the last item
     */
     ...
     * 
     * @param atomizing
     *            true if the caller of this iterator will atomize any nodes
     *            that are returned, and is therefore willing to accept the
     *            typed value of the nodes instead of the nodes themselves.
     */
     ...
     * 
     * @return the position in the array (zero-based) of the first item not
