...
public class NamePool implements Serializable {
    ...
    // user-defined names
    // will always have a fingerprint above this range, which can be tested by a
    // mask.
    ...
    /**
     * Get the singular static NamePool. Until Saxon 8.9, all Configurations
     * shared the same NamePool unless users explicitly allocated a different
     * one. This is no longer the case (each Configuration now has its own
     * NamePool), but the singular static NamePool remains available.
     * 
     * @return a singular NamePool, shared by all users of this Java VM
     *         instance.
     */
    ...
    /**
     * Set the default NamePool. NOTE: although a "default" namepool still
     * exists, it is actually no longer used by default. Since Saxon 8.9, when
     * you create a new Configuration, a new NamePool specific to that
     * configuration is created. The so-called "default" namepool is only used
     * if you specifically ask for it.
     * 
     * @param pool
     *            the NamePool to be returned by calls of
     *            {@link #getDefaultNamePool()}
     */
    ...
    public synchronized int allocateNamespaceCode(String prefix, String uri) {
        ...
        if (prefixCode != 0) {
            ...
            if (prefixes.length == 0
                    || (!prefixes[0].equals(prefix) && Arrays.asList(prefixes)
                            .indexOf(prefix) < 0)) {
                if (prefixes.length == MAX_PREFIXES_PER_URI) {
                    throw new NamePoolLimitException(
                            "NamePool limit exceeded: max "
     ...
     *         namespace code identifies both the prefix and the URI. Return -1
     *         if no namespace code has been allocated (in this case the caller
     *         should call allocateNamespaceCode() to get one).
     */
    ...
    /**
     * Get a prefix among all the prefixes used with a given URI, given its
     * index
     * 
     * @param uriCode
     ...
     *            required
     * @return the prefix with the given index. If the index is 0, the prefix is
     *         always "".
     */
        ...
        if (NamespaceConstant.isReserved(uri)) {
            ...
            if (fp != -1) {
                ...
                if (prefix == null || prefix.length() == 0) {
                    ...
                    final String[] prefixes = prefixesForUri[uriCode];
                    int prefixPosition = Arrays.asList(prefixes)
                            .indexOf(prefix);
                    if (prefixPosition < 0) {
                        if (prefixes.length == MAX_PREFIXES_PER_URI) {
                            throw new NamePoolLimitException(
    ...
    public synchronized int allocate(String prefix, short uriCode,
            String localName) {
        // System.err.println("Allocate " + prefix + " : " + uriCode + " : " +
        // localName);
    ...
    private int allocateInternal(String prefix, short uriCode, String localName) {
        ...
        // System.err.println("name code = " + prefixIndex + "/" + depth + "/" +
        // hash);
        ...
        if ((nameCode & USER_DEFINED_MASK) == 0) {
            ...
            if (uriCode == NamespaceConstant.XML_CODE) {
                ...
                if (prefix.length() == 0) {
                    ...
                    return prefix + ':'
                            + StandardNames.getLocalName(nameCode & FP_MASK);
                }
            }
        }
    ...
    public String getClarkName(int nameCode) {
        ...
        if (entry.uriCode == 0) {
            ...
            String n = '{' + getURIFromURICode(entry.uriCode) + '}'
                    + entry.localName;
            return n.intern();
        }
    }
    ...
    /**
     * Get the namespace URI code from a namespace code.
     * 
     * @param code
     *            a namespace code, representing the binding of a prefix to a
     *            namespace URI
     * @return the namespace URI code represented by this namespace binding
     */

    public int getURICodeFromNamespaceCode(int code) {
        return code & 0xffff;
    }
    ...
    /**
     * Get the namespace prefix code from a namespace code.
     * 
     * @param code
     *            a namespace code representing the binding of a prefix to a
     *            URI)
     * @return the prefix code represented by this namespace binding
     */

    public int getPrefixCodeFromNamespaceCode(int code) {
        // System.err.println("get prefix for " + code);
        return code >> 16;
    }
    ...
    public synchronized void diagnosticDump() {
        ...
        for (int u = 0; u < urisUsed; u++) {
            ...
            System.err
                    .println("Prefixes for URI " + u + " = " + fsb.toString());
        }
    }
    ...
    public synchronized void statistics() {
        ...
        System.err.println("NamePool contents: " + entries + " entries in "
                + slots + " chains. " + +prefixesUsed + " prefixes, "
                + urisUsed + " URIs");
    }
