...
/**
 * This module contains data regarding the classification of characters in XML
 * 1.0 and XML 1.1, and a number of interrogative methods to support queries on
 * this data.
 * 
 * For characters in the BMP, the information is tabulated by means of an array
 * of one-byte entries, one for each character, with bit-significant property
 * settings. For characters outside the BMP, the rules are built in to the
...
public class XMLCharacterData {
    ...
    /**
     * Bit setting to indicate that a character is valid at the start of an XML
     * 1.0 name
     */
    ...
    public static boolean isValid10(int i) {
        return i < 65536 ? (data[i] & VALID_10_MASK) != 0
                : (UTF16.NONBMP_MIN <= i && i <= UTF16.NONBMP_MAX);
    }
    ...
    /**
     * Determine whether a character is valid at the start of an NCName in XML
     * 1.0
     * 
     * @param i
     *            the character
     * @return true if the character is valid at the start of an NCName in XML
     *         1.0
     */
    ...
    public static boolean isValid11(int i) {
        return i < 65536 ? (data[i] & VALID_11_MASK) != 0
                : (UTF16.NONBMP_MIN <= i && i <= UTF16.NONBMP_MAX);
    }
    ...
    public static boolean isNCName11(int i) {
        return i < 65536 ? (data[i] & NAME_11_MASK) != 0
                : (UTF16.NONBMP_MIN <= i && i <= MAX_XML11_NAME_CHAR);
    }
    ...
    public static boolean isNCNameStart11(int i) {
        return i < 65536 ? (data[i] & NAME_START_11_MASK) != 0
                : (UTF16.NONBMP_MIN <= i && i <= MAX_XML11_NAME_CHAR);
    }
