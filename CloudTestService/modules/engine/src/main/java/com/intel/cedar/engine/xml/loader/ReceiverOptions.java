...
/**
 * ReceiverOptions defines a set of constants, which can be used in calls to
 * methods on the Receiver interface. The values are bit-significant.
 * 
...
public class ReceiverOptions {
    ...
    /**
     * Flag indicating that the value contains no special characters that need
     * to be escaped
     */
    ...
    /**
     * Flag used with character content that has been validated against a
     * nillable element declaration. Needed because of a peculiar rule for
     * validating xs:key values
     */
    ...
    /**
     * Flag indicating that the namespace (of an element or attribute name) has
     * already been declared; it does not need to be generated by the namespace
     * fixup process.
     */
    ...
    /**
     * Flag used with the characters() event to indicate that the event
     * represents an entire text node, that is, the text node has not been
     * fragmented over several characters() events
     */
    ...
    /**
     * Flag indicating that the ID/IDREF properties have been set if applicable:
     * if this bit is set, then the absence of the IS_ID bit means the node is
     * not an ID, and similarly for IS_IDREF
     */

    public static final int ID_IDREF_CHECKED = 8192;

    /**
     * Flag set on startDocument() in relation to an xsl:message call with
     * terminate="yes"
     */
