...
public abstract class ProxyReceiver implements Receiver {
    ...
    public void setSystemId(String systemId) {
        ...
        if (systemId != this.systemId) {
            // use of == rather than equals() is deliberate, since this is only
            // an optimization
    ...
    /**
     * Set the underlying receiver. This call is mandatory before using the
     * Receiver.
     * 
     * @param receiver
     *            the underlying receiver, the one that is to receive events
    ...
    public void setUnderlyingReceiver(Receiver receiver) {
        if (receiver != nextReceiver) {
            ...
            if (receiverConfiguration != null) {
                nextReceiver.setReceiverConfiguration(receiverConfiguration);
            }
        }
    }
    ...
    public void setReceiverConfiguration(ReceiverConfiguration config) {
        if (receiverConfiguration != config) {
            receiverConfiguration = config;
            if (nextReceiver != null) {
                nextReceiver.setReceiverConfiguration(config);
            }
        }
    }
    ...
    public NamePool getNamePool() {
        return receiverConfiguration.getConfiguration().getNamePool();
    }
    ...
    public void open() throws XMLException {
        if (nextReceiver == null) {
            throw new IllegalStateException(
                    "ProxyReceiver.open(): no underlying receiver provided");
        }
        nextReceiver.open();
    }
    ...
    public void close() throws XMLException {
        // TODO: this isn't safe. It's wrong to assume that because we've
        // finished writing to this
        // receiver, then we've also finished writing to other receivers in the
        // pipe.
        nextReceiver.close();
    }
     ...
     * parent element; however, duplicates may be reported. A namespace must not
     * conflict with any namespaces already used for element or attribute names.
     * 
     * @param namespaceCode
    ...
    public void processingInstruction(String target, CharSequence data,
            int locationId, int properties) throws XMLException {
        nextReceiver
                .processingInstruction(target, data, locationId, properties);
    }
