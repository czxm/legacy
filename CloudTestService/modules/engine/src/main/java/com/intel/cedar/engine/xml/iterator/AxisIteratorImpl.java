...
/**
 * A SequenceIterator is used to iterate over a sequence. An AxisIterator is a
 * SequenceIterator that always iterates over a set of nodes, and that throws no
 * exceptions; it also supports the ability to find the last() position, again
 * with no exceptions. This class is an abstract implementation of AxisIterator
 * that is used as a base class for many concrete implementations. The main
 * functionality that it provides is maintaining the current position.
 */
