 ...
 * 
 */
public final class ModelAdapterFactoryRegistry {
     ...
     */
    public ModelAdapterFactoryRegistry() {
        super();
    ...
    public void addFactory(IModelAdapterFactory factory) {
    ...
    public boolean contains(Object type) {
        ...
        for (int i = 0; i < internalList.size(); i++) {
            IModelAdapterFactory factory = (IModelAdapterFactory) internalList
                    .get(i);
     ...
     * this can not be used to add/remove factories. Its primarily provided for
     * those few cases where a list of factories must be copied from one model
    ...
    /**
     * This method is a not a pure resistry. Factories retrieved based on their
     * response to "isFactoryForType(type)". Note that if there is more than one
     * factory that can answer 'true' that the most recently added factory is
     * used.
     */
    public IModelAdapterFactory getFactoryFor(Object type) {

        IModelAdapterFactory result = null;
        if (factories == null)
            return null;
        ...
        for (int i = listSize - 1; i >= 0; i--) {
            ...
            // factory added last.
            IModelAdapterFactory a = (IModelAdapterFactory) factories.get(i);
    ...
    public void release() {
        ...
        for (int i = 0; i < localList.size(); i++) {
            IModelAdapterFactory a = (IModelAdapterFactory) localList.get(i);
            ...
            try {
                ...
                //Program problem releasing factory" + a, e); //$NON-NLS-1$
            }
        }
    }
     ...
     * is more than one, all are removed. If there is none, the call simply
     * returns (that is, it is not considered an error).
     */
    public void removeFactoriesFor(java.lang.Object type) {
        if (factories != null) {
            ...
            for (int i = listSize - 1; i >= 0; i--) {
                // It is the adapter factories responsibility to answer
                // isFactoryForType so it gets choosen.
                IModelAdapterFactory a = (IModelAdapterFactory) factories
                        .get(i);
                if (a.isFactoryForType(type)) {
                    factories.remove(a);
                }
    ...
    public void removeFactory(IModelAdapterFactory factory) {
