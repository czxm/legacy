...
abstract class TreeIterator implements AxisIterator, LookaheadIterator {
    ...
    /**
     * Create an axis Iterator for a given type and name of node, from a given
     * origin node
     * 
     * @param origin
     *            the node from which the axis originates
     * @param nodeTest
     *            test to be satisfied by the returned nodes, or null if all
     *            nodes are to be returned.
     */

    public TreeIterator(NodeImpl origin, NodeTest nodeTest) {
        next = origin;
        start = origin;
    ...
    /**
     * Test whether a node conforms to the node type and name constraints. Note
     * that this returns true if the supplied node is null, this is a way of
     * terminating a loop.
     */
    ...
    /**
     * Advance along the axis until a node is found that matches the required
     * criteria
     */
    ...
    /**
     * Advance one step along the axis: the resulting node might not meet the
