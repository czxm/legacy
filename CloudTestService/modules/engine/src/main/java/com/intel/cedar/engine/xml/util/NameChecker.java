...
import com.intel.cedar.engine.xml.QNameException;
...
/**
 * A NameChecker performs validation and analysis of XML names. There are two
 * implementations of this interface, one for XML 1.0 names and one for XML 1.1
 * names. The class also handles validation of characters against the XML 1.0 or
 * XML 1.1 rules.
 */
...
public abstract class NameChecker {
    ...
    /**
     * Validate whether a given string constitutes a valid QName, as defined in
     * XML Namespaces. Note that this does not test whether the prefix is
     * actually declared.
     * 
     * @param name
    ...
    /**
     * Validate whether a given string constitutes a valid NCName, as defined in
     * XML Namespaces.
     * 
     * @param name
     *            the name to be tested
    ...
    /**
     * Validate a QName, and return the prefix and local name. The local name is
     * checked to ensure it is a valid NCName. The prefix is not checked, on the
     * theory that the caller will look up the prefix to find a URI, and if the
     * prefix is invalid, then no URI will be found.
     * 
     * @param qname
     *            the lexical QName whose parts are required. Note that leading
     *            and trailing whitespace is not permitted
    ...
    public final String[] getQNameParts(CharSequence qname)
        ...
        if (colon < 0) {
            ...
            if (colon == 0) {
                throw new QNameException("QName cannot start with colon: "
                        + Err.wrap(qname));
            }
            if (colon == len - 1) {
                throw new QNameException("QName cannot end with colon: "
                        + Err.wrap(qname));
            }
            ...
            if (!isValidNCName(parts[1])) {
                throw new QNameException("Invalid QName local part "
                        + Err.wrap(parts[1]));
            }
        }
        return parts;
    ...
    /**
     * Validate a QName, and return the prefix and local name. Both parts are
     * checked to ensure they are valid NCNames.
     * <p/>
     * <p>
     * <i>Used from compiled code</i>
     * </p>
     * 
     * @param qname
     *            the lexical QName whose parts are required. Note that leading
    ...
    public final String[] checkQNameParts(CharSequence qname)
        ...
        if (parts[0].length() > 0 && !isValidNCName(parts[0])) {
            throw new QNameException("Invalid QName prefix "
                    + Err.wrap(parts[0]));
        }
        return parts;
    }
     ...
     * @param ch
     *            the character to be tested
     * @return true if this is a valid character in an NCName the selected
     *         version of XML
     */
     ...
     * @param ch
     *            the character to be tested
     * @return true if this is a valid character at the start of an NCName the
     *         selected version of XML
     */
