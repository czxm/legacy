...
environmentBlock9x(JNIEnv *env)
{
    ...
    jmethodID String_init_ID =
        (env)->GetMethodID( JNU_ClassString(env), "<init>", "([B)V");
    jbyteArray bytes;
    jbyte *blockA = (jbyte *) GetEnvironmentStrings();
    if (blockA == NULL) {
    ...
    if ((bytes = (env)->NewByteArray( i)) == NULL) return NULL;
    (env)->SetByteArrayRegion( bytes, 0, i, blockA);
    FreeEnvironmentStringsA((LPSTR)blockA);
    return (jstring)(env)->NewObject( JNU_ClassString(env),
                             String_init_ID, bytes);
}

/* Returns a Windows style environment block, discarding final trailing NUL */
JNIEXPORT jstring JNICALL
Java_com_intel_xml_rss_util_rexec_WindowsProcessEnvironment_environmentBlock(JNIEnv *env, jclass klass)
{
    int i;
    jstring envblock;
    ...
    envblock = (env)->NewString( blockW, i);
