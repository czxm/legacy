...
int
readSingle(JNIEnv *env, jobject that, jfieldID fid) {
    int nread;
    char ret;
...
int
readBytes(JNIEnv *env, jobject that, jbyteArray bytes,
          jint off, jint len, jfieldID fid)
{
    ...
    datalen = (env)->GetArrayLength( bytes);
...
void
writeSingle(JNIEnv *env, jobject that, jint byte, jfieldID fid) {
    char c = byte;
    int n;
    FD fd = GET_FD(that, fid);
    if (fd == -1) {
...
void
writeBytes(JNIEnv *env, jobject that, jbyteArray bytes,
          jint off, jint len, jfieldID fid)
{
    int n, datalen;
    char stackBuf[BUF_SIZE];
    ...
    (env)->GetByteArrayRegion( bytes, off, len, (jbyte *)buf);
