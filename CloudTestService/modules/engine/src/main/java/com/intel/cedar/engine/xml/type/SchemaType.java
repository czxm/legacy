 ...
 * 
 * <p>
 * There is a hierarchy of interfaces that extend SchemaType, representing the
 * top levels of the schema type system: SimpleType and ComplexType, with
 * SimpleType further subdivided into List, Union, and Atomic types.
 * </p>
 * 
 * <p>
 * The implementations of these interfaces are organized into a different
 * hierarchy: on the one side, built-in types such as AnyType, AnySimpleType,
 * and the built-in atomic types and list types; on the other side, user-defined
...
public interface SchemaType extends SchemaComponent {
    ...
    // DerivationMethods. These constants are copied from org.w3.dom.TypeInfo.
    // They are redefined here to avoid
    // creating a dependency on the TypeInfo class, which is only available when
    // JAXP 1.3 is available.
    ...
    /**
     * Derivation by substitution. This constant, unlike the others, is NOT
     * defined in the DOM level 3 TypeInfo interface.
     */
    ...
    SchemaType getBaseType() throws RuntimeException;
    ...
    /**
     * Get a description of this type for use in error messages. This is the
     * same as the display name in the case of named types; for anonymous types
     * it identifies the type by its position in a source schema document.
     * 
     * @return text identifing the type, for use in a phrase such as
     *         "the type XXXX".
     */
    ...
    /**
     * Check that this type is validly derived from a given type, following the
     * rules for the Schema Component Constraint
     * "Is Type Derivation OK (Simple)" (3.14.6) or
     * "Is Type Derivation OK (Complex)" (3.4.6) as appropriate.
     * 
     * @param base
     *            the base type; the algorithm tests whether derivation from
    ...
    public void checkTypeDerivationIsOK(SchemaType base, int block)
            throws XMLException;
