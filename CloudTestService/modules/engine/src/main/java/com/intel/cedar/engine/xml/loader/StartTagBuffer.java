...
/**
 * StartTagBuffer is a ProxyReceiver that buffers attributes and namespace
 * events within a start tag. It maintains details of the namespace context, and
 * a full set of attribute information, on behalf of other filters that need
 * access to namespace information or need to process attributes in arbitrary
 * order.
 * 
 * <p>
 * StartTagBuffer also implements namespace fixup (the process of creating
 * namespace nodes|bindings on behalf of constructed element and attribute
 * nodes). Although this would be done anyway, further down the pipeline, it has
 * to be done early in the case of a validating pipeline, because the namespace
 * bindings must be created before any namespace-sensitive attribute content is
 * validated.
 * </p>
 */
...
public class StartTagBuffer extends ProxyReceiver implements NamespaceResolver {
    ...
    // array holds a list of all namespaces currently declared (organised as
    // pairs of entries,
    // prefix followed by URI). The stack contains an entry for each element
    // currently open; the
    // value on the stack is an Integer giving the number of namespaces added to
    // the main
    // namespace stack by that element.

    private int[] namespaces = new int[50]; // all namespace codes currently
                                            // declared
    ...
    public void setReceiverConfiguration(ReceiverConfiguration config) {
        super.setReceiverConfiguration(config);
    ...
    public void startDocument(int properties) throws XMLException {
        // a document node in the content sequence of an element is ignored.
        // However, we need
        ...
        inDocument = true; // we ought to clear this on endDocument, but it only
                           // affects diagnostics
    }
    ...
    public void startElement(int nameCode, int typeCode, int locationId,
        ...
        // Record the current height of the namespace list so it can be reset at
        // endElement time
        ...
        // automatically by the caller (which is true, for example, for a
        // literal
        // result element).
    ...
    public void namespace(int namespaceCode, int properties)
        ...
        if (!acceptAttributes) {
            throw new XMLException(
                    "A namespace node ("
                            + getNamePool().getPrefixFromNamespaceCode(
                                    namespaceCode)
     ...
     * @param properties
     *            Bit significant value. The following bits are defined: <dd>
     *            DISABLE_ESCAPING</dd> <dt>Disable escaping for this attribute</dt>
     *            <dd>NO_SPECIAL_CHARACTERS</dd> <dt>Attribute value contains no
     *            special characters</dt>
    ...
    public void attribute(int nameCode, int typeCode, CharSequence value,
        ...
        bufferedAttributes.addAttribute(nameCode, typeCode, value.toString(),
                locationId, properties);

        // Note: we're relying on the fact that AttributeCollection can hold two
        // attributes of the same name
        // and maintain their order, because the check for duplicate attributes
        // is not done until later in the
        // pipeline. We validate both the attributes (see Bugzilla #4600 which
        // proposes to legitimize this.)
     ...
     * 
     * @param nscode
     *            the namepool namespace code for the declaration
     */
    ...
    /**
     * startContent: Add any namespace undeclarations needed to stop namespaces
     * being inherited from parent elements
     */

    public void startContent() throws XMLException {
        nextReceiver.startElement(elementNameCode, elementTypeCode,
        ...
        for (int i = 0; i < length; i++) {
            nextReceiver.attribute(bufferedAttributes.getNameCode(i),
                    bufferedAttributes.getTypeAnnotation(i), bufferedAttributes
                            .getValue(i), 0, bufferedAttributes
                            .getProperties(i)
                            | ReceiverOptions.NAMESPACE_OK);
        }
        acceptAttributes = false;
    ...
    protected void declareNamespacesForStartElement() throws XMLException {
        for (int i = namespacesSize - countStack[depth - 1]; i < namespacesSize; i++) {
            nextReceiver.namespace(namespaces[i], 0);
        }
    }

    protected void declareAllNamespaces() throws XMLException {
     ...
     * @return the 16-bit URI code, or -1 if the prefix is not bound to any
     *         namespace
     */
    ...
    protected short getURICode(short prefixCode) {
        for (int i = namespacesSize - 1; i >= 0; i--) {
            if ((namespaces[i] >> 16) == (prefixCode)) {
                ...
                if (uriCode == 0) {
                    // we've found a namespace undeclaration, so it's as if the
                    // prefix weren't there at all
                    break;
                }
                return uriCode;
            }
     ...
     * allocating a substitute prefix if not. The prefix is acceptable unless a
     * namespace declaration has been written that assignes this prefix to a
     * different namespace URI. This method also checks that the element or
     * attribute namespace has been declared, and declares it if not.
     * 
     * @param nameCode
     *            the proposed element or attribute name
     * @param seq
     *            sequence number of attribute, used for generating distinctive
     *            prefixes
     * @return the actual allocated name, which may be different.
     */

    private int checkProposedPrefix(int nameCode, int seq) throws XMLException {
        ...
        if (existingURICode == -1) {
            ...
            if ((nscode & 0xffff) == existingURICode) {
                ...
                int newCode = namePool.allocate(prefix, namePool
                        .getURI(nameCode), namePool.getLocalName(nameCode));
    ...
    /**
     * It is possible for a single output element to use the same prefix to
     * refer to different namespaces. In this case we have to generate an
     * alternative prefix for uniqueness. The one we generate is based on the
     * sequential position of the element/attribute: this is designed to ensure
     * both uniqueness (with a high probability) and repeatability
     * 
     * @param nscode
     *            the namespace code of the proposed element or attribute name
     * @param seq
     *            sequence number of attribute, used for generating distinctive
     *            prefixes
     * @return the actual allocated name, which may be different.
