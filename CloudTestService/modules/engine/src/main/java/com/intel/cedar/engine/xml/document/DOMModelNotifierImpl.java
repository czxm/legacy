...
public class DOMModelNotifierImpl implements DOMModelNotifier {
    ...
    public void attrReplaced(Element element, Attribute newAttr,
        ...
        NodeImpl notifier = (NodeImpl) element;
    ...
    public void childReplaced(Node parentNode, Node newChild, Node oldChild) {
        if (parentNode == null)
            return;
        NodeImpl notifier = (NodeImpl) parentNode;
    ...
    public void editableChanged(Node node) {
        if (node == null)
            return;
        NodeImpl notifier = (NodeImpl) node;
        int offset = notifier.getStartOffset();
    ...
    public void endChanging() {
        ...
        if (this.changedRoot != null) {
            ...
            /*
             * if (Debug.debugNotifyDeferred) { String p =
             * this.changedRoot.getNodeName();
             * System.out.println("Deferred STRUCTURE_CHANGED: " + p);
             * //$NON-NLS-1$ }
             */
    ...
    private void notify(INodeNotifier notifier, int eventType,
        ...
        if (this.changing && !this.flushing) {
            ...
            if ((!doingNewModel)
                    || (((Node) notifier).getNodeKind() == Node.DOCUMENT)) {
    ...
    private void notifyDeferred() {
            ...
            while (it.hasNext()) {
                ...
                for (int i = 0; i < es.length - 1; i++) {
                    ...
                    if (es[i].type == INodeNotifier.ADD
                        ...
                        next.discarded = true;
                        /*
                         * if (Debug.debugNotifyDeferred) { event.reason =
                         * event.reason + ADDED_THEN_REMOVED + "(see " +
                         * next.index + ")"; //$NON-NLS-1$ //$NON-NLS-2$
                         * next.reason = next.reason + ADDED_THEN_REMOVED +
                         * "(see " + event.index + ")"; //$NON-NLS-1$
                         * //$NON-NLS-2$ }
                         */
                    }
                }
                        ...
                        if (es != null)
                            for (int p = 0; p < es.length
                                ...
                                if (prev.type == INodeNotifier.REMOVE
                                     ...
                                     * PARENT_IS_REPARENTED + "(see " +
                                     * prev.index + ")"; //$NON-NLS-1$
                                     * //$NON-NLS-2$ }
                                     */
                                    break;
                                     ...
                                     * if (Debug.debugNotifyDeferred) {
                                     * event.reason = event.reason +
                                     * PARENT_IS_ADDED + "(see " + prev.index +
                                     * ")"; //$NON-NLS-1$ //$NON-NLS-2$ }
                                     */
                                    break;
                                }
                            ...
                            for (int n = 0; n < es.length; n++) {
                                ...
                                if (next.index > event.index
                                    ...
                                    if (next.oldValue == event.notifier) {
                                         ...
                                         * if (Debug.debugNotifyDeferred) {
                                         * event.reason = event.reason +
                                         * PARENT_IS_REMOVED_TOO + "(see " +
                                         * next.index + ")"; //$NON-NLS-1$
                                         * //$NON-NLS-2$ }
                                         */
                                        break;
                                    }
        ...
        /*
         * if (Debug.debugNotifyDeferred) { for (int l = 0; l < count; l++) {
         * NotifyEvent event = (NotifyEvent) this.fEvents.get(l); Object o =
         * null; String t = null; if (event.type == INodeNotifier.ADD) { o =
         * event.newValue; t = " + "; //$NON-NLS-1$ } else if (event.type ==
         * INodeNotifier.REMOVE) { o = event.oldValue; t = " - "; //$NON-NLS-1$
         * } if (o instanceof Element) { String p = ((Node)
         * event.notifier).getNodeName(); String c = ((Node) o).getNodeName();
         * String d = (event.discarded ? "! " : "  "); //$NON-NLS-1$
         * //$NON-NLS-2$ System.out.println(d + p + t + c); } } }
         */
        this.flushing = false;
        this.fEvents = null;
    }
    ...
    private void setCommonRootIfNeeded(Node node) {
        ...
        if (this.changedRoot == null) {
            ...
            if (changedRoot.getNodeKind() != Node.DOCUMENT
                    && changedRoot != node) {
    ...
    public void structureChanged(Node node) {
        ...
        if (isChanging()) {
             ...
             * if (Debug.debugNotifyDeferred) { String p =
             * this.changedRoot.getNodeName();
             * System.out.println("requested STRUCTURE_CHANGED: " + p);
             * //$NON-NLS-1$ }
             */
            return;
        }
        /*
         * if (Debug.debugNotifyDeferred) { String p = node.getNodeName();
    ...
    public void valueChanged(Node node) {
        if (node == null)
            return;
        NodeImpl notifier = null;
        if (node.getNodeKind() == Node.ATTRIBUTE) {
            Attribute attr = (Attribute) node;
            notifier = (NodeImpl) attr.getOwnerElement();
