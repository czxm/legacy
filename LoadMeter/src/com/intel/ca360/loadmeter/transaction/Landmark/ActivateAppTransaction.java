...
public class ActivateAppTransaction extends LandmarkRestTransaction {
	public ActivateAppTransaction(HttpClient client, List<ParamType> params) {
		super(client, params);
	}
	...
	protected List<Action> createActions(Action start){
		...
		final APIChecker appInActive = new APIChecker();		
		result.add(new DynamicAction(HTTP_METHOD.GET, "getAppSum") {
			@Override
			public Object[] getArguments(){
				return new String[]{INSTANCE.idp, authnChecker.getResponse(), "", "", "INCOMPLETE"};
			}
			@Override
		...
		result.add(new DynamicAction(HTTP_METHOD.PUT, "updateApp"){
			...
			public boolean skipped(){
				...
				app = Util.stringRegexMatch(theAppIdMatcher, appInActive.getResponse());
				return app == null;
			}
			@Override
			public Object[] getArguments(){
			...
			public String getRequestBody(){
				return String.format(updateAppJSON, app, "ACTIVE");
			}			
			@Override
			public Header[] getHeaders(){
