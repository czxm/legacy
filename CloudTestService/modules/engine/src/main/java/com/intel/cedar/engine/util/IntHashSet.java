...
public class IntHashSet implements IntSet, Serializable {
     ...
     */
    public static final int MAX_SIZE = 1 << NBIT; // maximum number of keys
                                                  // mapped
    ...
    // private double _factor; // 0.0 <= _factor <= 1.0 - changed by MHK to
    // assume factor = 0.25
    ...
    /**
     * Gets the index of the value, if it exists, or the index at which this
     * value would be added if it does not exist yet.
     */
    ...
    private void setCapacity(int capacity) {
        ...
        if (values != null) {
            for (int i = 0; i < nold; ++i) {
                int value = values[i];
                if (value != ndv) {
                    // Don't use add, because the capacity is necessarily large
                    // enough,
                    // and the value is necessarily unique (since in this set
                    // already)!
                    // add(values[i]);
                    ++_size;
