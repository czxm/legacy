...
import com.intel.soak.plugin.hcatalog.load.hadoop.HadoopUtils;
 ...
 * the number of threads over time and repeatedly execute the task {@link com.intel.soak.plugin.hcatalog.load.tasks.Task}
...
public class HadoopLoadGenerator extends Configured implements Tool {
    ...
    public SortedMap<Long, ReduceResult> runLoadTest(String confFileName, Configuration conf) throws Exception, MetaException, TException {
        ...
        String tokenStr = "";
        try{
            tokenStr = hiveClient.getDelegationToken("mapred");
        }catch(Exception e){
            e.printStackTrace();
        }
