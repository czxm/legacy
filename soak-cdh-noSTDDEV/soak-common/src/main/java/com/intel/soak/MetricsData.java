...
public class MetricsData implements Serializable {
    ...
    private void specialMerge(Aggregator aggr, Float d){
        if(Aggregator.MED.equals(aggr)) {
            ...
            if(values[Aggregator.MED.ordinal()] != null){
                appendData(values[Aggregator.MED.ordinal()]);
                values[Aggregator.MED.ordinal()] = null;
            }
            appendData(d);
        }
        ...
        if (Aggregator.MED.matches(this.aggregators)) {
            if (this.dataIndex > 0) {
                ...
                if (toSort.length % 2 == 0) {
                    med = (toSort[toSort.length / 2 - 1] + toSort[toSort.length / 2]) / 2;
                } else {
                    med = toSort[toSort.length / 2];
                }
